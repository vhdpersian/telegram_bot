...
import io.github.nixtabyte.telegram.jtelebot.response.json.InlineQueryResult;
...
public final class TelegramRequestFactory {
	 ...
	 */
	public static TelegramRequest createSendMessageRequest(final String chatId,final String text, final boolean disableWebPagePreview, final String replyToMessageId, final CustomReplyKeyboard customReplyKeyboard) throws JsonParsingException{
	...
	public static TelegramRequest createAnswerInlineQueryRequest(final String queryId, final InlineQueryResult[] inlineQueryResult) throws JsonParsingException{
		List <BasicNameValuePair>basicNameValuePair = new ArrayList<BasicNameValuePair>();
		basicNameValuePair.add(new BasicNameValuePair("inline_query_id",queryId));
		
		if(inlineQueryResult!=null){
			try {
				final String jsonString = MapperHandler.INSTANCE.getObjectMapper().writeValueAsString(inlineQueryResult);
				basicNameValuePair.add(new BasicNameValuePair("results",jsonString));
			} catch (IOException e) {
				throw new JsonParsingException(e);
			} 
		}
		return new TelegramRequest(RequestType.SET_ANSWER_INLINE_QUERY,basicNameValuePair);
	}
